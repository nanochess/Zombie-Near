        ...
        ;
        ; (c) Copyright Oscar Toledo Guti‚rrez 2011-2012
        ;
        ...
        ; Revisi¢n: 21-jun-2012. Adaptaci¢n a Memotech MTX-512.
        ; Revisi¢n: 22-jun-2012. Se quita logo de distribuidor y se pone mi
        ;                        logo. Se integra pantalla alterna de t¡tulo 
        ;                        para cuando se gana el juego. Se agrega
        ;                        aceleraci¢n para que parezca correr a 60 hz.
        ;                        Adaptaci¢n a Tatung Einstein.
        ; Revisi¢n: 25-jun-2012. Ya funciona versi¢n para Tatung Einstein,
        ;                        que resulta que no conecta INT del VDP.
        ; Revisi¢n: 26-jun-2012. Ajustes para Tatung Einstein.
        ; Revisi¢n: 02-nov-2012. Correcci¢n de un bug al empezar, iniciaba el
        ;                        sonido antes de descompactar m£sica en VRAM.
        ; Revisi¢n: 05-mar-2015. Nueva versi¢n Memotech que corre de CP/M
        ;                        normal sin LOADRUN.
        ; Revisi¢n: 14-mar-2015. En Memotech se mejora el mapeado del
        ;                        controlador 2 para su uso "est ndar" de
        ;                        teclas compatible con joystick (ZCBM+Y)
        ;

        ;
        ; Defina a 1 para ensamblar para Memotech.
        ; Defina a 0 para ensamblar para Tatung Einstein.
        ;
MEMOTECH:       EQU 1

        ;
        ; Si MEMOTECH es 1, defina esto a 1 para CP/M normal
        ;
CPM:            EQU 0

        ;
        ; Defina esto para activar vidas infinitas
        ;
PROGRAMADOR:    EQU 0
        ...
        ;
        ; Mapa de uso del ROM:
        ; $4000-$bFFF - Programa, mapas, gr ficas y sonido
        ;
    ...
    if MEMOTECH=1
        ;
        ; Crea un archivo RUN que puede ser ejecutado por el sistema
        ; operativo SDX del MEMOTECH y tambi‚n por la utilidad
        ; LOADRUN.COM que corre en CP/M.
        ;
      if CPM=1
        FNAME "ZOMBNEAR.COM"
        ORG $0100
        ...
        dw inicio_rom
        dw final_rom-inicio_rom
      endif

    else
        ;
        ; Crea un archivo COM que puede ser ejecutado por el
        ; sistema operativo XTAL del Tatung Einstein (una copia
        ; del CP/M)
        ;
        FNAME "ZOMBNEAR.COM"
        ORG $0100
...
VDP:    equ MEMOTECH*$01+(NOT MEMOTECH AND 1)*$08         ; Puerta del VDP
CTC:    equ MEMOTECH*$08+(NOT MEMOTECH AND 1)*$28         ; Base del Z80-CTC
PSG:    equ MEMOTECH*$06+(NOT MEMOTECH AND 1)*$02         ; Chip de sonido
KEYSEL: equ $05                      ; Selecci¢n del teclado
KEY1:   equ $05                      ; Byte 1 de teclado
KEY2:   equ $06                      ; Byte 2 de teclado

inicio_rom:
        JP INICIO       ; Direcci¢n de inicio del cartucho ROM
...
espera_int:
    if memotech=1
        halt
    else        ; El Tatung Einstein no conecta el INT del VDP
        push af
        in a,(VDP+1)
        nop
        in a,(VDP+1)
...
.2:     ld (hl),0
    ...
    if memotech=1
        ...
        out (CTC+2),a
        out (CTC+3),a
        ld a,$08        ; Aparentemente offset de interrupci¢n
        out (CTC+0),a
    if MEMOTECH=1
        ld a,$25        ; Desactiva interrupci¢n canal 2
        out (CTC+2),a
        ld a,$9c        ; Cada 156 ticks
        out (CTC+2),a
        ld a,$25        ; Desactiva interrupci¢n canal 1
        out (CTC+1),a
        ld a,$9c        ; Cada 156 ticks
        out (CTC+1),a
        ld a,$c5        ; Activa interrupci¢n canal 0 y como contador
        out (CTC+0),a
        ld a,$01        ; Un tick
        out (CTC+0),a
        in a,(VDP+1)
...
.1:     ld hl,ciclo
        ...
        sub 6
        jr nz,$+5
        ld (hl),a               ; Agrega un cuadro cada 5
        jr $+6

        call espera_int         ; Actualizaci¢n, tambi‚n ahorra energ¡a.
...
vector_presentacion:
        ...
        ld hl,modo
        bit 2,(hl)      ; ¨Definir sprites (modo de juego) ?
        jr z,vector_quieto  ; No, salta.
        bit 5,(hl)      ; ¨En helic¢ptero?
        jr nz,vector_quieto  ; S¡, salta.
        ...
        ;
vector_quieto:
        ; El Memotech siempre fue de 50 hz.
    if 0
...
visual_mensaje_idioma:
        LD C,$A1                ; Verdes para letra inicial
        ;
...
VISUAL_LETRA2:
        ...
        SRL H
        RR L
        LD DE,$3800-$0400       ; H ten¡a bit 5 puesto
        ADD HL,DE
        LD A,L
        call WRTVRM
        POP HL
...
.2:
        ...
        call dos_digitos
        ld a,$20
        call WRTVRM     ; Un cero de relleno
...
maneja_entrada:
        ...
        jp nz,.2        ; No, controla jugador 2
        call .2
        ; jp .1

        ;
        ; El jugador 1 con teclado / controlador derecho
        ;
.1:     ld ix,jug1      ; Jugador 1
    ...
    if MEMOTECH=1
        ...
        ;
        ; El jugador 2 con teclado / controlador izq.
        ;
...
.26:    ld a,(ix+d_sprite)
        ...
        ld (ix+d_energia),75    ; 75 impactos en mapa 3.
...
.17:    ldi
        ...
        ; Diccionario 8
.5:
        call lee_bit    ; lee_bit  0 - Sin cambio
        jr z,.6
        call lee_bit    ; lee_bit  100 - S¡mbolo 0
        call lee_bit    ; lee_bit  101 - S¡mbolo 1
        ex af,af'
        and 3
        cp 2
        jr c,.15
        ex af,af'
        call lee_bit    ; lee_bit  1100 - S¡mbolo 2
        ex af,af' ;        1101 - S¡mbolo 3
        and 3     ;        1110 - S¡mbolo 4
        add a,2  
        cp 5
        jr nz,.15
        call lee_bit    ; lee_bit  11110 - S¡mbolo 5
        ex af,af'
        and 1
        ld a,5
        jr z,.15
        call lee_bit    ; lee_bit  111110 - S¡mbolo 6
...
.7:
        call lee_bit    ; lee_bit  0 - Sin cambio
        jr z,.8
        call lee_bit    ; lee_bit  1xx - S¡mbolos 0-3
        jr nz,.9
        call lee_bit    ; lee_bit
        call lee_bit    ; lee_bit
        ex af,af'
        and 3
        jr .10
...
.9:
        call lee_bit    ; lee_bit  11xxx - S¡mbolos 4-10
        call lee_bit    ; lee_bit
        call lee_bit    ; lee_bit
        ex af,af'
        and 7
        ...
        cp 11
        jr nz,.10
        call lee_bit    ; lee_bit  11111xx - S¡mbolos 11-14
        call lee_bit    ; lee_bit
        ex af,af'
        and 3
        add a,11
...
jug2_inicial:           ; La chica
        ...
        ;
        ; Descompactador Pletter-0.5c (XL2S Entertainment y Team Bomba)
        ; La £nica forma como consegu¡ meter este juego en 32K
        ;
unpack:
        call zona_dura
; VRAM address setup
...
graf_bitmap:
        incbin "combina1.bin.plet5"
graf_color:
        incbin "combina2.bin.plet5"
figuras_sprites:
        incbin "combina3.bin.plet5"
color_letras:
        incbin "combina4.bin.plet5"

logo_1:
        incbin "logo1.bin.plet5"
logo_2:
        incbin "logo2.bin.plet5"
logo_21:
        incbin "logo21.bin.plet5"
logo_22:
        incbin "logo22.bin.plet5"
fin_1:
        incbin "fin1.bin.plet5"
fin_2:
        incbin "fin2.bin.plet5"
        ; M£sica
musica:
        incbin "musica.bin.plet5"
...
nmi:    RB 1    ; NMI recibido en mal momento
...
ef_envo:        RB 2    ; Frecuencia voz 4
ef_envm:        RB 1    ; Modo envolvente
ef_jeje:        RB 1    ; Volumen voz 4
ef_cont:        RB 1    ; Valor del registro de control
