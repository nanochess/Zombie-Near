...
void compacta_cortina(char *ap1, char *ap2, FILE *salida)
{
  ...
  while (d < tam_y) {
    if (c >= 0) {
      compacta3(ap1 + (c & 7) + ((c >> 3) * tam_x), 0xff, salida);
      compacta3(ap2 + (c & 7) + ((c >> 3) * tam_x), 0x01, salida);
    }
    compacta3(ap1 + (d & 7) + ((d >> 3) * tam_x), 0xff, salida);
    compacta3(ap2 + (d & 7) + ((d >> 3) * tam_x), 0x01, salida);
    c--;
    d++;
  }
}
...
void compacta_interlazado(char *ap1, char *ap2, FILE *salida)
{
  ...
  for (c = 0; c < 8; c++) {
    for (d = 0; d < tam_y / 8; d++) {
      compacta3(ap1 + c + d * tam_x, 0xff, salida);
      compacta3(ap2 + c + d * tam_x, 0x01, salida);
    }
  }
}
...
#if 1  /* Busca repetici¢n de patr¢n, solo eficaz con inc == 1 porque */
