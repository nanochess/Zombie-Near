...
** Revisi¢n: 08-abr-2011. Ahora busca las tiras repetidas en la salida
**                        descompactada en vez de compactada, también las busca
**                        mientras checa bytes no repetidos, excelente mejora
**                        de compresión.
*/
...
void compacta2(unsigned char *ap1, int tam, FILE *salida, int inc)
{
...
#endif
#if 1  /* Busca tiras repetidas en salida, muy efectivo */
    inicio_tira = 0;
    largo_tira = -1;
    if (inc == 1) {
      cuenta = ap1 - base - 256;
      if (cuenta < 0)
        cuenta = 0;
      for (; cuenta < ap1 - base; cuenta++) {
        por_comparar = (ap2 - ap1) / inc;
        maximo = ap1 - base - cuenta;
        if (por_comparar > maximo)
          por_comparar = maximo;
        for (estimado = 0; estimado < por_comparar; estimado++) {
          if (base[cuenta + estimado] != ap1[estimado * inc])
            break;
        }
        if (estimado > largo_tira) {
          largo_tira = estimado;
          inicio_tira = cuenta;
        }
      }
  ...
  *ap4++ = 0;  /* Marcador de final */
  ...
  fprintf(salida, "        ; Revisi¢n: 08-abr-2011. Ahora incluye color y caracteres.\n");
  fprintf(salida, "        ;\n");
  fprintf(salida, "\n");
  fprintf(salida, "sprites_heroes:     equ $c000\n");
  fprintf(salida, "figuras_sprites:    equ $c4c0\n");
  fprintf(salida, "sprites_jefes:      equ $cbc0\n");
  fprintf(salida, "letras:             equ $d7c0\n");
  fprintf(salida, "color_letras:       equ $d980\n");
  fprintf(salida, "graf_bitmap:        equ $db40\n");
  fprintf(salida, "graf_color:         equ $e180\n");
  fprintf(salida, "\n");
