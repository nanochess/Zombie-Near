...
** XL2S Entertainment
**
** Es un compresor tipo LZ77, la compresión por bits puede ser lenta
** comparada con RLE (especialmente si se descompacta directo a VRAM porque
** tiene que leer el VRAM), pero siempre le va a ganar en reducción de
** espacio.
**
** Revisión: 19-abr-2011. Convertido a C por Óscar Toledo G.
*/
...
*/
struct metadata {
  unsigned int reeks;           /* Número de veces que se repite el byte */
  unsigned int cpos[7];
  unsigned int clen[7];
} *m;

/*
** Compresión por byte para cada modo
*/
struct pakdata {
  unsigned int cost;
  unsigned int mode;
...
/*
** Agrega un número variable a la salida
*/
void addvar(int i)
{
  int j = 0x8000;
...
/*
** Anota donde va a guardar los próximos 8 bits
*/
void claimevent(void)
{
  s.ep = s.dp;
...
/*
** Anota el costo de usar un número variable
*/
...
void createmetadata(void)
{
  ...
  /*
  ** Para acelerar la búsqueda de cadenas concordantes
  **
  ...
  ** por bytes individuales porque el offset ocupa mínimo un byte.
  **
  ** last es una especie de tabla de dispersión indicando donde se halla
  ...
  */
  ...
  /*
  ** Cuenta los bytes repetidos a partir de cada posición del origen
  */
  r = -1;
  t = 0;
  ...
  for (bl = 0; bl != 7; ++bl) {
    ...
    */
    for (i = 0; i < tam_origen; ++i) {
      ...
      */
      while ((p = prev[p]) != -1) {
        if (i - p > maxlen[bl])  /* ¿Excede el offset posible? */
          break;                 /* Sí, termina */
        l = 0;
        while (d[p + l] == d[i + l] && (i + l) < tam_origen) {

          /*
          ** Truco de aceleración:
          **   Si la cadena que se quiere substituir tiene bytes repetidos...
          **   y la cadena que se usará como reemplazo tiene menos bytes
          **   repetidos...
          **   entonces puede tomar solamente los que hay y además se
          **   evita el clásico problema o(n2)
          */
          if (m[i + l].reeks > 1) {
        ...
        if (l > m[i].clen[bl]) {  /* Busca la cadena más larga */
          m[i].clen[bl] = l;      /* Longitud */
          m[i].cpos[bl] = i - p;  /* Posición (offset) */
        }
      }
    }
    putchar('.');
...
/*
** Obtiene el tamaño final del archivo con un modo
*/
int getlen(struct pakdata *p, unsigned int modo)
{
  ...
  /*
  ** Truco, marcha de adelante para atrás, así puede conocer todas las
  ** combinaciones posibles de compactación.
  */
  for (i = tam_origen - 1; i != -1; --i) {
    kmode = 0;  /* Literal */
    kl = 0;
    ...
    /* Prueba todos los tamaños hasta hallar el más corto */
    j = m[i].clen[0];
    while (j > 1) {
