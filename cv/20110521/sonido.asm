...
.10:    add hl,hl               ; Baja 2 octavas
        add hl,hl
        ; Demasiado para el chip de sonido de Coleco, ­qu‚ l stima!
;        ld a,(ef_t)
;        add a,a
;        ld d,a
;        ld a,(ef_ta)
...
.1:     ld a,(de)
        ...
        and $3f
        out (PSG),a
        ld a,(de)       ; Tiene un volumen 0 (m¡nimo) - 15 (m ximo)
        cpl             ; Pone en formato SN76489 (exactamente al rev‚s)
        and $1f         ; Deja un 1 en el bit 4 para formar reg. de volumen
        or c
        out (PSG),a
        xor a           ; Lo reinicia
        ld (de),a
        ...
        or $ec
        ex de,hl
        cp (hl)         ; ef_rp ¨El mismo valor que antes?
        ex de,hl
        jr z,.2         ; S¡, entonces salta para evitar reiniciar LFSR.
        ld (de),a
        out (PSG),a
.2:     inc de
        ld a,(de)       ; ef_mezc
...
tabla_notas:
        ...
        dw 0
        ; Octava 2 - 1
        dw 1023,1023,1023,1023,1023,1023,1023,1023,1023,1017,956,909
        ; Octava 3 - 13
