...
** Revisi¢n: 02-jul-2013. Maneja cuatro habitaciones extras por mapa.
*/
...
int main(void)
{
  ...
  cientificos = 0;
  vidas = 0;
  entrada = fopen("mapa4.asm", "r");
  base = bytes;
  fprintf(salida_js, "map4 = [\n");
  fprintf(salida, "\nmapa4:\n");
  for (c = 1; c < 107; c++)
    crujido(c);
  fprintf(salida_js, "];\n");
  ...
  printf("Zombies: %d, cient¡ficos: %d, vidas: %d\n",
    zombies, cientificos, vidas);
  printf("Mapa 4 reducido a %d bytes\n", bytes - base);
#endif
  fclose(salida);
...
void crujido(int piso)
{
  ...
  */
  ...
  fprintf(salida_js, "[");
  for (c = 0; c < 72; c++) {
    fprintf(salida_js, (c == 71) ? "%d" : "%d,", mapa[c]);
  }
  fprintf(salida_js, "],\n");
