...
void compacta_interlazado(char *ap1, char *ap2, FILE *salida)
{
  ...
  for (c = 0; c < 8; c++) {
    for (d = 0; d < tam_y / 8; d++) {
      compacta2(ap1 + c + d * tam_x, salida, 8);
      compacta2(ap2 + c + d * tam_x, salida, 8);
    }
  }
}
...
#if 1  /* Busca repetici¢n de patr¢n */
    if (inc == 1 && ap1 + 3 < ap2) {
      for (maximo = 8; maximo >= 2; maximo--) {
        cuenta = 0;
        while (cuenta < 14) {
          if (ap1 + (cuenta + 2) * maximo > ap2
           || memcmp(ap1 + (cuenta * maximo),
                     ap1 + (cuenta + 1) * maximo, maximo) != 0)
            break;
          cuenta++;
        }
        if (cuenta > 0) {
